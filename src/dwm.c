...
drawbar(Monitor *m)
{
    ...
    if (m == selmon) {
        ...
        drw_text(drw, m->ww - tw - sidepad, vertpad, tw, bh - vertpad * 2, 0, stext, 0);
    }
    ...
    for (i = 0; i < LENGTH(tags); i++) {
        ...
        drw_text(drw, x, vertpadinside, w, bh - vertpadinside * 2, lrpad / 2, tags[i], urg & 1 << i);
        if (occ & 1 << i)
            drw_rect(drw, x + boxs, boxs + vertpad, boxw, boxw,
    ...
    w = TEXTW(m->ltsymbol);
    drw_setscheme(drw, scheme[SchemeNorm]);
    x = drw_text(drw, x, vertpad, w, bh - vertpadinside * 2, lrpad / 2, m->ltsymbol, 0);

    if ((w = m->ww - tw - x - sidepadinside) > bh) {
        if (m->sel) {
            drw_setscheme(drw, scheme[m == selmon ? SchemeSel : SchemeNorm]);
            drw_text(drw, x, vertpadinside, w, bh - vertpadinside * 2, lrpad / 2, m->sel->name, 0);
            if (m->sel->isfloating)
                drw_rect(drw, x + boxs, boxs + vertpadinside, boxw, boxw, m->sel->isfixed, 0);
        } else {
            drw_setscheme(drw, scheme[SchemeNorm]);
            drw_rect(drw, x, vertpadinside, w, bh - vertpadinside * 2, 1, 1);
        }
    }
...
slidein(Client *c)
{
    int steps = 40; // more steps = smoother
    ...
    for (int i = 0; i <= steps; i++) {
        ...
        XSync(dpy, False);
        usleep(1000 * 8); // smaller = faster animation
    }
...
{
    Client *c;
    static int spawnx = 60, spawny = 60;
    int step = 40;
    ...
    for (c = m->clients; c; c = c->next) {
        ...
        if (!c->isfloating) {
            ...
            c->x = m->wx + spawnx;
            c->y = m->wy + spawny;
            resize(c, c->x, c->y, winw - 2 * c->bw, winh - 2 * c->bw, 1);
            spawnx += step;
            spawny += step;
            if (spawnx + winw > m->wx + m->ww - 100)
                spawnx = 60;
            if (spawny + winh > m->wy + m->wh - 100)
                spawny = 60;
        }
    }
}
...
dwindle(Monitor *m)
{
    ...
    /* Unfloat all visible windows first */
    for (c = m->clients; c; c = c->next)
        if (ISVISIBLE(c))
            c->isfloating = 0;
    ...
    int gappx_inner  = 10;  // gap between windows
    int gappx_top    = 35;   // gap from top edge of screen
    int gappx_bottom = 15;  // gap from bottom edge
    int gappx_left   = 15;   // gap from left edge
    int gappx_right  = 15;   // gap from right edge

    int x = m->wx + gappx_left;
    int y = m->wy + gappx_top;
    int w = m->ww - gappx_left - gappx_right;
    int h = m->wh - gappx_top - gappx_bottom;

    int toggle = 0;

    for (c = nexttiled(m->clients); c; c = nexttiled(c->next)) {
        if (toggle == 0) {
            int neww = (n == 1) ? w : (w - gappx_inner) / 2;
            resize(c, x, y, neww - 2 * c->bw, h - 2 * c->bw, 0);
            x += neww + gappx_inner;
            w -= neww + gappx_inner;
        } else {
            int newh = (n == 1) ? h : (h - gappx_inner) / 2;
...
setup(void)
{
	...
	lrpad = drw->fonts->h;
	bh = drw->fonts->h + vertpadinside * 2;
	updategeom();
	/* init atoms */
...
updatebarpos(Monitor *m)
{
	...
	if (m->showbar) {
	    if (m->topbar) {
	        m->by = m->wy + vertpad;
	        m->wy += bh + vertpad * 2;   // push windows down
	        m->wh -= bh + vertpad * 2;
	    } else {
	        m->by = m->wy + m->wh - bh - vertpad;
	        m->wh -= bh + vertpad * 2;   // push windows up
	    }
	    m->wx += sidepad;
	    m->ww -= sidepad * 2;
	} else {
...
view(const Arg *arg)
{
    ...
    if (newtag == selmon->tagset[selmon->seltags])
        return;
